...
{
    public sealed class AuthoriseViewModel : AnalyticsBase, IDisposable
    {
        ...
        private void Authorise()
        {
            ...
            Action authorise = async () =>
            {
                try
                {
                    cancellationTokenSource = new CancellationTokenSource();

                    user = await DrumbleApi.Authorise(cancellationTokenSource.Token, user, email, pin);

                    if (user.IsBumbleRegistered)
                    {
                        base.HideHeaderLoader();
                        OneTimePinTextBoxIsEnabled = true;

                        AuthoriseMessage.Send(user, Messages.Enums.AuthoriseMessageReason.Authorised);
            ...
            DispatcherHelper.CheckBeginInvokeOnUI(authorise);
        }
